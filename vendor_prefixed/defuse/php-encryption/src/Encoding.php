...
final class Encoding
{
    ...
    {
        $hex = '';
        $len = \YoastSEO_Vendor\Defuse\Crypto\Core::ourStrlen($byte_string);
        for ($i = 0; $i < $len; ++$i) {
    ...
    public static function hexToBin($hex_string)
    {
        $hex_pos = 0;
        $bin = '';
        $hex_len = \YoastSEO_Vendor\Defuse\Crypto\Core::ourStrlen($hex_string);
        $state = 0;
        $c_acc = 0;
        while ($hex_pos < $hex_len) {
            ...
            if (($c_num0 | $c_alpha0) === 0) {
                throw new \YoastSEO_Vendor\Defuse\Crypto\Exception\BadFormatException('Encoding::hexToBin() input is not a hex string.');
            }
    ...
    {
        $length = \YoastSEO_Vendor\Defuse\Crypto\Core::ourStrlen($string);
        if ($length < 1) {
            return '';
        }
        do {
        ...
        return (string) \YoastSEO_Vendor\Defuse\Crypto\Core::ourSubstr($string, 0, $length);
    }
    /*
    ...
    public static function saveBytesToChecksummedAsciiSafeString($header, $bytes)
    {
        ...
        \YoastSEO_Vendor\Defuse\Crypto\Core::ensureTrue(\YoastSEO_Vendor\Defuse\Crypto\Core::ourStrlen($header) === self::SERIALIZE_HEADER_BYTES, 'Header must be ' . self::SERIALIZE_HEADER_BYTES . ' bytes.');
        return \YoastSEO_Vendor\Defuse\Crypto\Encoding::binToHex($header . $bytes . \hash(self::CHECKSUM_HASH_ALGO, $header . $bytes, \true));
    }
    /**
    ...
    public static function loadBytesFromChecksummedAsciiSafeString($expected_header, $string)
    {
        // Headers must be a constant length to prevent one type's header from
        // being a prefix of another type's header, leading to ambiguity.
        \YoastSEO_Vendor\Defuse\Crypto\Core::ensureTrue(\YoastSEO_Vendor\Defuse\Crypto\Core::ourStrlen($expected_header) === self::SERIALIZE_HEADER_BYTES, 'Header must be 4 bytes.');
        ...
        $bytes = \YoastSEO_Vendor\Defuse\Crypto\Encoding::hexToBin($string);
        /* Make sure we have enough bytes to get the version header and checksum. */
        if (\YoastSEO_Vendor\Defuse\Crypto\Core::ourStrlen($bytes) < self::SERIALIZE_HEADER_BYTES + self::CHECKSUM_BYTE_SIZE) {
            throw new \YoastSEO_Vendor\Defuse\Crypto\Exception\BadFormatException('Encoded data is shorter than expected.');
        }
        /* Grab the version header. */
        $actual_header = (string) \YoastSEO_Vendor\Defuse\Crypto\Core::ourSubstr($bytes, 0, self::SERIALIZE_HEADER_BYTES);
        if ($actual_header !== $expected_header) {
            throw new \YoastSEO_Vendor\Defuse\Crypto\Exception\BadFormatException('Invalid header.');
        }
        /* Grab the bytes that are part of the checksum. */
        $checked_bytes = (string) \YoastSEO_Vendor\Defuse\Crypto\Core::ourSubstr($bytes, 0, \YoastSEO_Vendor\Defuse\Crypto\Core::ourStrlen($bytes) - self::CHECKSUM_BYTE_SIZE);
        /* Grab the included checksum. */
        $checksum_a = (string) \YoastSEO_Vendor\Defuse\Crypto\Core::ourSubstr($bytes, \YoastSEO_Vendor\Defuse\Crypto\Core::ourStrlen($bytes) - self::CHECKSUM_BYTE_SIZE, self::CHECKSUM_BYTE_SIZE);
        /* Re-compute the checksum. */
        $checksum_b = \hash(self::CHECKSUM_HASH_ALGO, $checked_bytes, \true);
        /* Check if the checksum matches. */
        if (!\YoastSEO_Vendor\Defuse\Crypto\Core::hashEquals($checksum_a, $checksum_b)) {
            throw new \YoastSEO_Vendor\Defuse\Crypto\Exception\BadFormatException("Data is corrupted, the checksum doesn't match");
        }
        return (string) \YoastSEO_Vendor\Defuse\Crypto\Core::ourSubstr($bytes, self::SERIALIZE_HEADER_BYTES, \YoastSEO_Vendor\Defuse\Crypto\Core::ourStrlen($bytes) - self::SERIALIZE_HEADER_BYTES - self::CHECKSUM_BYTE_SIZE);
    }
}
