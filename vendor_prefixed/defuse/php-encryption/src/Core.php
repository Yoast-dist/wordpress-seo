...
final class Core
{
    ...
    {
        \YoastSEO_Vendor\Defuse\Crypto\Core::ensureTrue(\YoastSEO_Vendor\Defuse\Crypto\Core::ourStrlen($ctr) === \YoastSEO_Vendor\Defuse\Crypto\Core::BLOCK_BYTE_SIZE, 'Trying to increment a nonce of the wrong size.');
        \YoastSEO_Vendor\Defuse\Crypto\Core::ensureTrue(\is_int($inc), 'Trying to increment nonce by a non-integer.');
        // The caller is probably re-using CTR-mode keystream if they increment by 0.
        \YoastSEO_Vendor\Defuse\Crypto\Core::ensureTrue($inc > 0, 'Trying to increment a nonce by a nonpositive amount');
        \YoastSEO_Vendor\Defuse\Crypto\Core::ensureTrue($inc <= \PHP_INT_MAX - 255, 'Integer overflow may occur');
        /*
         * We start at the rightmost byte (big-endian)
         * So, too, does OpenSSL: http://stackoverflow.com/a/3146214/2224584
         */
        for ($i = \YoastSEO_Vendor\Defuse\Crypto\Core::BLOCK_BYTE_SIZE - 1; $i >= 0; --$i) {
            $sum = \ord($ctr[$i]) + $inc;
            /* Detect integer overflow and fail. */
            \YoastSEO_Vendor\Defuse\Crypto\Core::ensureTrue(\is_int($sum), 'Integer overflow in CTR mode nonce increment');
    ...
    public static function secureRandom($octets)
    {
        ...
        try {
            ...
            throw new \YoastSEO_Vendor\Defuse\Crypto\Exception\EnvironmentIsBrokenException('Your system does not have a secure random number generator.');
        }
    }
    /**
    ...
    public static function HKDF($hash, $ikm, $length, $info = '', $salt = null)
    {
        ...
        $digest_length = \YoastSEO_Vendor\Defuse\Crypto\Core::ourStrlen(\hash_hmac($hash, '', '', \true));
        // Sanity-check the desired output length.
        \YoastSEO_Vendor\Defuse\Crypto\Core::ensureTrue(!empty($length) && \is_int($length) && $length >= 0 && $length <= 255 * $digest_length, 'Bad output length requested of HDKF.');
        // "if [salt] not provided, is set to a string of HashLen zeroes."
        if (\is_null($salt)) {
            $salt = \str_repeat("\0", $digest_length);
        }
        // HKDF-Extract:
        // PRK = HMAC-Hash(salt, IKM)
        // The salt is the HMAC key.
        $prk = \hash_hmac($hash, $ikm, $salt, \true);
        // HKDF-Expand:
        // This check is useless, but it serves as a reminder to the spec.
        \YoastSEO_Vendor\Defuse\Crypto\Core::ensureTrue(\YoastSEO_Vendor\Defuse\Crypto\Core::ourStrlen($prk) >= $digest_length);
        // T(0) = ''
        $t = '';
        $last_block = '';
        for ($block_index = 1; \YoastSEO_Vendor\Defuse\Crypto\Core::ourStrlen($t) < $length; ++$block_index) {
            // T(i) = HMAC-Hash(PRK, T(i-1) | info | 0x??)
            $last_block = \hash_hmac($hash, $last_block . $info . \chr($block_index), $prk, \true);
            // T = T(1) | T(2) | T(3) | ... | T(N)
            $t .= $last_block;
        }
        // ORM = first L octets of T
        /** @var string $orm */
        $orm = \YoastSEO_Vendor\Defuse\Crypto\Core::ourSubstr($t, 0, $length);
        \YoastSEO_Vendor\Defuse\Crypto\Core::ensureTrue(\is_string($orm));
        return $orm;
    }
    /**
    ...
    public static function hashEquals($expected, $given)
    {
        ...
        // length.
        \YoastSEO_Vendor\Defuse\Crypto\Core::ensureTrue(\YoastSEO_Vendor\Defuse\Crypto\Core::ourStrlen($expected) === \YoastSEO_Vendor\Defuse\Crypto\Core::ourStrlen($given));
        $blind = \YoastSEO_Vendor\Defuse\Crypto\Core::secureRandom(32);
        $message_compare = \hash_hmac(\YoastSEO_Vendor\Defuse\Crypto\Core::HASH_FUNCTION_NAME, $given, $blind);
        $correct_compare = \hash_hmac(\YoastSEO_Vendor\Defuse\Crypto\Core::HASH_FUNCTION_NAME, $expected, $blind);
    ...
    {
        \YoastSEO_Vendor\Defuse\Crypto\Core::ensureTrue(\defined($name), 'Constant ' . $name . ' does not exists');
    }
    /**
    ...
    {
        \YoastSEO_Vendor\Defuse\Crypto\Core::ensureTrue(\function_exists($name), 'function ' . $name . ' does not exists');
    }
    /**
    ...
    {
        if (!$condition) {
            throw new \YoastSEO_Vendor\Defuse\Crypto\Exception\EnvironmentIsBrokenException($message);
        }
    }
    /*
    ...
    public static function ourStrlen($str)
    {
        ...
        if ($exists) {
            ...
            \YoastSEO_Vendor\Defuse\Crypto\Core::ensureTrue($length !== \false);
            return $length;
        } else {
            return \strlen($str);
        }
    ...
    public static function ourSubstr($str, $start, $length = null)
    {
        ...
        $input_len = \YoastSEO_Vendor\Defuse\Crypto\Core::ourStrlen($str);
        ...
        if ($exists) {
            ...
            if (\YoastSEO_Vendor\Defuse\Crypto\Core::ourStrlen($substr) > $length || \YoastSEO_Vendor\Defuse\Crypto\Core::ourStrlen($substr) === 0 && $length !== 0 && $start !== $input_len) {
                throw new \YoastSEO_Vendor\Defuse\Crypto\Exception\EnvironmentIsBrokenException('Your version of PHP has bug #66797. Its implementation of
    ...
    public static function pbkdf2($algorithm, $password, $salt, $count, $key_length, $raw_output = \false)
    {
        ...
        \YoastSEO_Vendor\Defuse\Crypto\Core::ensureTrue(\in_array($algorithm, \hash_algos(), \true), 'Invalid or unsupported hash algorithm.');
        // Whitelist, or we could end up with people using CRC32.
        $ok_algorithms = ['sha1', 'sha224', 'sha256', 'sha384', 'sha512', 'ripemd160', 'ripemd256', 'ripemd320', 'whirlpool'];
        \YoastSEO_Vendor\Defuse\Crypto\Core::ensureTrue(\in_array($algorithm, $ok_algorithms, \true), 'Algorithm is not a secure cryptographic hash function.');
        \YoastSEO_Vendor\Defuse\Crypto\Core::ensureTrue($count > 0 && $key_length > 0, 'Invalid PBKDF2 parameters.');
        ...
        $hash_length = \YoastSEO_Vendor\Defuse\Crypto\Core::ourStrlen(\hash($algorithm, '', \true));
        ...
        for ($i = 1; $i <= $block_count; $i++) {
            ...
            $last = $salt . \pack('N', $i);
            // first iteration
            $last = $xorsum = \hash_hmac($algorithm, $last, $password, \true);
            // perform the other $count - 1 iterations
            for ($j = 1; $j < $count; $j++) {
                /**
                 * @psalm-suppress InvalidOperand
                 */
                $xorsum ^= $last = \hash_hmac($algorithm, $last, $password, \true);
            }
            $output .= $xorsum;
        }
        if ($raw_output) {
            return (string) \YoastSEO_Vendor\Defuse\Crypto\Core::ourSubstr($output, 0, $key_length);
        } else {
            return \YoastSEO_Vendor\Defuse\Crypto\Encoding::binToHex((string) \YoastSEO_Vendor\Defuse\Crypto\Core::ourSubstr($output, 0, $key_length));
        }
    }
}
