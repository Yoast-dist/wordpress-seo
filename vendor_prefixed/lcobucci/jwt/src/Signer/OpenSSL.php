...
abstract class OpenSSL extends \YoastSEO_Vendor\Lcobucci\JWT\Signer\BaseSigner
{
    public function createHash($payload, \YoastSEO_Vendor\Lcobucci\JWT\Signer\Key $key)
    {
        ...
        try {
            ...
            if (!\openssl_sign($payload, $signature, $privateKey, $this->getAlgorithm())) {
                throw \YoastSEO_Vendor\Lcobucci\JWT\Signer\CannotSignPayload::errorHappened(\openssl_error_string());
            }
            return $signature;
        } finally {
    ...
    {
        if (!\is_resource($key)) {
            throw \YoastSEO_Vendor\Lcobucci\JWT\Signer\InvalidKeyProvided::cannotBeParsed(\openssl_error_string());
        }
        $details = \openssl_pkey_get_details($key);
        if (!isset($details['key']) || $details['type'] !== $this->getKeyType()) {
            throw \YoastSEO_Vendor\Lcobucci\JWT\Signer\InvalidKeyProvided::incompatibleKey();
        }
    }
    /**
