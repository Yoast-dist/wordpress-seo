...
use YoastSEO_Vendor\Lcobucci\JWT\Claim\Factory as ClaimFactory;
...
use YoastSEO_Vendor\Lcobucci\JWT\Token\RegisteredClaimGiven;
...
class Builder
{
     ...
     */
    public function __construct(\YoastSEO_Vendor\Lcobucci\JWT\Parsing\Encoder $encoder = null, \YoastSEO_Vendor\Lcobucci\JWT\Claim\Factory $claimFactory = null)
    {
        $this->encoder = $encoder ?: new \YoastSEO_Vendor\Lcobucci\JWT\Parsing\Encoder();
    ...
    {
        $claim = \YoastSEO_Vendor\Lcobucci\JWT\Token\RegisteredClaims::AUDIENCE;
    ...
    private function convertToDate($value)
    {
        if (!$value instanceof \DateTimeImmutable) {
            \trigger_error('Using integers for registered date claims is deprecated, please use DateTimeImmutable objects instead.', \E_USER_DEPRECATED);
    ...
    protected function setRegisteredClaim($name, $value, $replicate)
    {
        ...
        if ($replicate) {
            \trigger_error('Replicating claims as headers is deprecated and will removed from v4.0. Please manually set the header if you need it replicated.', \E_USER_DEPRECATED);
    ...
    {
        if (\in_array($name, \YoastSEO_Vendor\Lcobucci\JWT\Token\RegisteredClaims::ALL, \true)) {
            \trigger_error('The use of the method "withClaim" is deprecated for registered claims. Please use dedicated method instead.', \E_USER_DEPRECATED);
        }
    ...
    {
        switch ($name) {
            case \YoastSEO_Vendor\Lcobucci\JWT\Token\RegisteredClaims::ID:
                return $this->identifiedBy($value);
            case \YoastSEO_Vendor\Lcobucci\JWT\Token\RegisteredClaims::EXPIRATION_TIME:
                return $this->expiresAt($value);
            case \YoastSEO_Vendor\Lcobucci\JWT\Token\RegisteredClaims::NOT_BEFORE:
                return $this->canOnlyBeUsedAfter($value);
            case \YoastSEO_Vendor\Lcobucci\JWT\Token\RegisteredClaims::ISSUED_AT:
                return $this->issuedAt($value);
            case \YoastSEO_Vendor\Lcobucci\JWT\Token\RegisteredClaims::ISSUER:
                return $this->issuedBy($value);
            case \YoastSEO_Vendor\Lcobucci\JWT\Token\RegisteredClaims::AUDIENCE:
     ...
     * @return Builder
     */
    public function sign(\YoastSEO_Vendor\Lcobucci\JWT\Signer $signer, $key)
    {
        if (!$key instanceof \YoastSEO_Vendor\Lcobucci\JWT\Signer\Key) {
     ...
     * @return Token
     */
    public function getToken(\YoastSEO_Vendor\Lcobucci\JWT\Signer $signer = null, \YoastSEO_Vendor\Lcobucci\JWT\Signer\Key $key = null)
    {
        if ($signer === null || $key === null) {
            \trigger_error('Not specifying the signer and key to Builder#getToken() is deprecated. Please move the arguments from Builder#sign() to Builder#getToken().', \E_USER_DEPRECATED);
        }
        $signer = $signer ?: $this->signer;
        $key = $key ?: $this->key;
        if ($signer instanceof \YoastSEO_Vendor\Lcobucci\JWT\Signer) {
            $signer->modifyHeader($this->headers);
        }
        $headers = new \YoastSEO_Vendor\Lcobucci\JWT\Token\DataSet($this->headers, $this->encoder->base64UrlEncode($this->encoder->jsonEncode($this->convertItems($this->headers))));
        $claims = new \YoastSEO_Vendor\Lcobucci\JWT\Token\DataSet($this->claims, $this->encoder->base64UrlEncode($this->encoder->jsonEncode($this->convertItems($this->claims))));
        return new \YoastSEO_Vendor\Lcobucci\JWT\Token($headers, $claims, $this->createSignature($headers->toString() . '.' . $claims->toString(), $signer, $key), ['', ''], $this->claimFactory);
    }
    /**
    ...
    private function convertItems(array $items)
    {
        ...
        $audience = \YoastSEO_Vendor\Lcobucci\JWT\Token\RegisteredClaims::AUDIENCE;
     ...
     * @return Signature
     */
    private function createSignature($payload, \YoastSEO_Vendor\Lcobucci\JWT\Signer $signer = null, \YoastSEO_Vendor\Lcobucci\JWT\Signer\Key $key = null)
    {
        if ($signer === null || $key === null) {
            return \YoastSEO_Vendor\Lcobucci\JWT\Signature::fromEmptyData();
        }
        $hash = $signer->sign($payload, $key)->hash();
        return new \YoastSEO_Vendor\Lcobucci\JWT\Signature($hash, $this->encoder->base64UrlEncode($hash));
    }
}
