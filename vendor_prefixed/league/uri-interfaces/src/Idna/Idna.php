...
use YoastSEO_Vendor\League\Uri\Exceptions\IdnaConversionFailed;
...
final class Idna
{
    ...
    private static function supportsIdna() : void
    {
        ...
        if (!$idnSupport) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\IdnSupportMissing('IDN host can not be processed. Verify that ext/intl is installed for IDN support and that ICU is at least version 4.6.');
        }
    }
    /**
     ...
     */
    public static function toAscii(string $domain, int $options) : \YoastSEO_Vendor\League\Uri\Idna\IdnaInfo
    {
        ...
        if (1 === \preg_match(self::REGEXP_IDNA_PATTERN, $domain)) {
            ...
            if ([] === $idnaInfo) {
                return \YoastSEO_Vendor\League\Uri\Idna\IdnaInfo::fromIntl(['result' => \strtolower($domain), 'isTransitionalDifferent' => \false, 'errors' => self::validateDomainAndLabelLength($domain)]);
            }
            /* @var array{errors: int, isTransitionalDifferent: bool, result: string} $idnaInfo */
            return \YoastSEO_Vendor\League\Uri\Idna\IdnaInfo::fromIntl($idnaInfo);
        }
        $error = self::ERROR_NONE;
        if (1 !== \preg_match(self::REGEXP_REGISTERED_NAME, $domain)) {
            $error |= self::ERROR_DISALLOWED;
        }
        return \YoastSEO_Vendor\League\Uri\Idna\IdnaInfo::fromIntl(['result' => \strtolower($domain), 'isTransitionalDifferent' => \false, 'errors' => self::validateDomainAndLabelLength($domain) | $error]);
    }
    /**
     ...
     */
    public static function toUnicode(string $domain, int $options) : \YoastSEO_Vendor\League\Uri\Idna\IdnaInfo
    {
        $domain = \rawurldecode($domain);
        if (\false === \stripos($domain, 'xn--')) {
            return \YoastSEO_Vendor\League\Uri\Idna\IdnaInfo::fromIntl(['result' => $domain, 'isTransitionalDifferent' => \false, 'errors' => self::ERROR_NONE]);
        }
        self::supportsIdna();
        /* @param-out array{errors: int, isTransitionalDifferent: bool, result: string} $idnaInfo */
        \idn_to_utf8($domain, $options, \INTL_IDNA_VARIANT_UTS46, $idnaInfo);
        if ([] === $idnaInfo) {
            throw \YoastSEO_Vendor\League\Uri\Exceptions\IdnaConversionFailed::dueToInvalidHost($domain);
        }
        /* @var array{errors: int, isTransitionalDifferent: bool, result: string} $idnaInfo */
        return \YoastSEO_Vendor\League\Uri\Idna\IdnaInfo::fromIntl($idnaInfo);
    }
