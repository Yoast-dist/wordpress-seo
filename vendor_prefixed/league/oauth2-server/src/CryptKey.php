...
class CryptKey
{
     ...
     */
    public function __construct($keyPath, $passPhrase = null, $keyPermissionsCheck = \true)
    {
        ...
        if ($keyPermissionsCheck === \true) {
            // Verify the permissions of the key
            $keyPathPerms = \decoct(\fileperms($this->keyPath) & 0777);
            if (\in_array($keyPathPerms, ['400', '440', '600', '640', '660'], \true) === \false) {
                \trigger_error(\sprintf('Key file "%s" permissions are not correct, recommend changing to 600 or 660 instead of %s', $this->keyPath, $keyPathPerms), \E_USER_NOTICE);
            }
        }
    }
    /**
    ...
    private function isValidKey($contents, $passPhrase)
    {
        ...
        return $details !== \false && \in_array($details['type'] ?? -1, [\OPENSSL_KEYTYPE_RSA, \OPENSSL_KEYTYPE_EC], \true);
    }
    /**
     * Retrieve key path.
     *
