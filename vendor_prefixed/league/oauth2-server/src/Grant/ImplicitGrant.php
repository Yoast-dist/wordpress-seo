...
class ImplicitGrant extends \YoastSEO_Vendor\League\OAuth2\Server\Grant\AbstractAuthorizeGrant
{
    /**
     * @var DateInterval
     */
    ...
    {
        return isset($request->getQueryParams()['response_type']) && $request->getQueryParams()['response_type'] === 'token' && isset($request->getQueryParams()['client_id']);
    }
    /**
     * {@inheritdoc}
     */
    public function validateAuthorizationRequest(\YoastSEO_Vendor\Psr\Http\Message\ServerRequestInterface $request)
    {
        ...
        } elseif (\is_array($client->getRedirectUri()) && \count($client->getRedirectUri()) !== 1 || empty($client->getRedirectUri())) {
        ...
        $scopes = $this->validateScopes($this->getQueryStringParameter('scope', $request, $this->defaultScope), $redirectUri);
        $stateParameter = $this->getQueryStringParameter('state', $request);
        if ($stateParameter !== null && !\is_string($stateParameter)) {
            throw \YoastSEO_Vendor\League\OAuth2\Server\Exception\OAuthServerException::invalidRequest('state');
        }
    ...
    public function completeAuthorizationRequest(\YoastSEO_Vendor\League\OAuth2\Server\RequestTypes\AuthorizationRequest $authorizationRequest)
    {
        ...
        if ($authorizationRequest->isAuthorizationApproved() === \true) {
            ...
            $finalizedScopes = $this->scopeRepository->finalizeScopes($authorizationRequest->getScopes(), $this->getIdentifier(), $authorizationRequest->getClient(), $authorizationRequest->getUser()->getIdentifier());
            $accessToken = $this->issueAccessToken($this->accessTokenTTL, $authorizationRequest->getClient(), $authorizationRequest->getUser()->getIdentifier(), $finalizedScopes);
            $response = new \YoastSEO_Vendor\League\OAuth2\Server\ResponseTypes\RedirectResponse();
            $response->setRedirectUri($this->makeRedirectUri($finalRedirectUri, ['access_token' => (string) $accessToken, 'token_type' => 'Bearer', 'expires_in' => $accessToken->getExpiryDateTime()->getTimestamp() - \time(), 'state' => $authorizationRequest->getState()], $this->queryDelimiter));
            return $response;
        }
