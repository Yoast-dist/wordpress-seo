...
class BearerTokenResponse extends \YoastSEO_Vendor\League\OAuth2\Server\ResponseTypes\AbstractResponseType
{
    /**
     * {@inheritdoc}
     */
    public function generateHttpResponse(\YoastSEO_Vendor\Psr\Http\Message\ResponseInterface $response)
    {
        ...
        $responseParams = ['token_type' => 'Bearer', 'expires_in' => $expireDateTime - \time(), 'access_token' => (string) $this->accessToken];
        if ($this->refreshToken instanceof \YoastSEO_Vendor\League\OAuth2\Server\Entities\RefreshTokenEntityInterface) {
            $refreshTokenPayload = \json_encode(['client_id' => $this->accessToken->getClient()->getIdentifier(), 'refresh_token_id' => $this->refreshToken->getIdentifier(), 'access_token_id' => $this->accessToken->getIdentifier(), 'scopes' => $this->accessToken->getScopes(), 'user_id' => $this->accessToken->getUserIdentifier(), 'expire_time' => $this->refreshToken->getExpiryDateTime()->getTimestamp()]);
        ...
        $response = $response->withStatus(200)->withHeader('pragma', 'no-cache')->withHeader('cache-control', 'no-store')->withHeader('content-type', 'application/json; charset=UTF-8');
     ...
     * @return array
     */
    protected function getExtraParams(\YoastSEO_Vendor\League\OAuth2\Server\Entities\AccessTokenEntityInterface $accessToken)
    {
        return [];
    }
}
