 ...
 */
namespace YoastSEO_Vendor\League\OAuth2\Server\RedirectUriValidators;

use YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError;
use YoastSEO_Vendor\League\Uri\Uri;
class RedirectUriValidator implements \YoastSEO_Vendor\League\OAuth2\Server\RedirectUriValidators\RedirectUriValidatorInterface
{
    /**
     * @var array
     */
    ...
    {
        try {
            $uri = \YoastSEO_Vendor\League\Uri\Uri::createFromString($redirectUri);
        } catch (\YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError $e) {
            return \false;
        }
        return $uri->getScheme() === 'http' && \in_array($uri->getHost(), ['127.0.0.1', '[::1]'], \true);
    }
    /**
    ...
    {
        return \in_array($redirectUri, $this->allowedRedirectUris, \true);
    }
    /**
    ...
    {
        $uri = \YoastSEO_Vendor\League\Uri\Uri::createFromString($url);
