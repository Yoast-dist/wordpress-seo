...
final class UriResolver
{
    ...
    public static function resolve($uri, $base_uri)
    {
        ...
        $null = $uri instanceof \YoastSEO_Vendor\Psr\Http\Message\UriInterface ? '' : null;
        if ($null !== $uri->getScheme()) {
            return $uri->withPath(self::removeDotSegments($uri->getPath()));
        }
        if ($null !== $uri->getAuthority()) {
            return $uri->withScheme($base_uri->getScheme())->withPath(self::removeDotSegments($uri->getPath()));
        }
        $user = $null;
        $pass = null;
        ...
        return $uri->withPath(self::removeDotSegments($uri_path))->withQuery($uri_query)->withHost($base_uri->getHost())->withPort($base_uri->getPort())->withUserInfo((string) $user, $pass)->withScheme($base_uri->getScheme());
    }
    /**
    ...
    {
        if (!$uri instanceof \YoastSEO_Vendor\League\Uri\Contracts\UriInterface && !$uri instanceof \YoastSEO_Vendor\Psr\Http\Message\UriInterface) {
    ...
    private static function removeDotSegments(string $path) : string
    {
        ...
        $new_path = \implode('/', \array_reduce($old_segments, [\YoastSEO_Vendor\League\Uri\UriResolver::class, 'reducer'], []));
    ...
    private static function resolvePathAndQuery($uri, $base_uri) : array
    {
        ...
        $null = $uri instanceof \YoastSEO_Vendor\Psr\Http\Message\UriInterface ? '' : null;
        $baseNull = $base_uri instanceof \YoastSEO_Vendor\Psr\Http\Message\UriInterface ? '' : null;
    ...
    private static function getComponent(string $method, $uri) : ?string
    {
        ...
        if ($uri instanceof \YoastSEO_Vendor\Psr\Http\Message\UriInterface && '' === $component) {
            return null;
        }
        return $component;
    }
    ...
    private static function formatHost($uri)
    {
        ...
        return $uri->withHost((string) \YoastSEO_Vendor\League\Uri\Uri::createFromComponents(['host' => $host])->getHost());
    }
    /**
    ...
    {
        return !\YoastSEO_Vendor\League\Uri\UriInfo::isRelativePath($uri) && self::componentEquals('getScheme', $uri, $base_uri) && self::componentEquals('getAuthority', $uri, $base_uri);
    }
    /**
    ...
    private static function relativizePath(string $path, string $basepath) : string
    {
        ...
        return self::formatPath(\str_repeat('../', \count($base_segments)) . \implode('/', $target_segments), $basepath);
    }
    /**
    ...
    {
        if ('' === $path) {
            return \in_array($basepath, ['', '/'], \true) ? $basepath : './';
        }
