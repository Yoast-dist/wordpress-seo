...
namespace YoastSEO_Vendor\League\Uri\UriTemplate;

use YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError;
use YoastSEO_Vendor\League\Uri\Exceptions\TemplateCanNotBeExpanded;
...
final class Expression
{
    ...
    private function __construct(string $operator, \YoastSEO_Vendor\League\Uri\UriTemplate\VarSpecifier ...$varSpecifiers)
    {
    ...
    {
        $mapper = static function (\YoastSEO_Vendor\League\Uri\UriTemplate\VarSpecifier $varSpecifier) : string {
    ...
    {
        $mapper = static function (\YoastSEO_Vendor\League\Uri\UriTemplate\VarSpecifier $variable) : string {
    ...
    public static function createFromString(string $expression) : self
    {
        if (1 !== \preg_match(self::REGEXP_EXPRESSION, $expression, $parts)) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError('The expression "' . $expression . '" is invalid.');
        }
        ...
        if ('' !== $parts['operator'] && \false !== \strpos(self::RESERVED_OPERATOR, $parts['operator'])) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError('The operator used in the expression "' . $expression . '" is reserved.');
        }
        $mapper = static function (string $varSpec) : VarSpecifier {
            return \YoastSEO_Vendor\League\Uri\UriTemplate\VarSpecifier::createFromString($varSpec);
        };
        return new \YoastSEO_Vendor\League\Uri\UriTemplate\Expression($parts['operator'], ...\array_map($mapper, \explode(',', $parts['variables'])));
    }
    /**
    ...
    public function expand(\YoastSEO_Vendor\League\Uri\UriTemplate\VariableBag $variables) : string
    {
        $parts = [];
     ...
     */
    private function replace(\YoastSEO_Vendor\League\Uri\UriTemplate\VarSpecifier $varSpec, \YoastSEO_Vendor\League\Uri\UriTemplate\VariableBag $variables) : string
    {
     ...
     */
    private function inject($value, \YoastSEO_Vendor\League\Uri\UriTemplate\VarSpecifier $varSpec, bool $useQuery) : array
    {
        if (\is_string($value)) {
     ...
     * @return array{0:string, 1:bool}
     */
    private function replaceString(string $value, \YoastSEO_Vendor\League\Uri\UriTemplate\VarSpecifier $varSpec, bool $useQuery) : array
    {
        if (':' === $varSpec->modifier()) {
     ...
     * @return array{0:string, 1:bool}
     */
    private function replaceList(array $value, \YoastSEO_Vendor\League\Uri\UriTemplate\VarSpecifier $varSpec, bool $useQuery) : array
    {
        if ([] === $value) {
            return ['', \false];
        }
        if (':' === $varSpec->modifier()) {
            throw \YoastSEO_Vendor\League\Uri\Exceptions\TemplateCanNotBeExpanded::dueToUnableToProcessValueListWithPrefix($varSpec->name());
        }
        $pairs = [];
    ...
    {
        static $delimiters = [':', '/', '?', '#', '[', ']', '@', '!', '$', '&', '\'', '(', ')', '*', '+', ',', ';', '='];
        static $delimitersEncoded = ['%3A', '%2F', '%3F', '%23', '%5B', '%5D', '%40', '%21', '%24', '%26', '%27', '%28', '%29', '%2A', '%2B', '%2C', '%3B', '%3D'];
