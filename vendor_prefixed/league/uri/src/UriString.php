...
final class UriString
{
     ...
     */
    private const ZONE_ID_ADDRESS_BLOCK = "þ€";
    /**
    ...
    public static function parse($uri) : array
    {
        ...
        if (1 === \preg_match(self::REGEXP_INVALID_URI_CHARS, $uri)) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('The uri `%s` contains invalid characters', $uri));
        }
        ...
        if (':' === $parts['scheme'] || 1 !== \preg_match(self::REGEXP_URI_SCHEME, $parts['scontent'])) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('The uri `%s` contains an invalid scheme', $uri));
        }
        if ('' === $parts['scheme'] . $parts['authority'] && 1 === \preg_match(self::REGEXP_INVALID_PATH, $parts['path'])) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('The uri `%s` contains an invalid path.', $uri));
        }
        /** @var array{scheme:?string, user:?string, pass:?string, host:?string, port:?int, path:string, query:?string, fragment:?string} $components */
    ...
    private static function filterHost(string $host) : string
    {
        ...
        if (!self::isIpHost(\substr($host, 1, -1))) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('Host `%s` is invalid : the IP host is malformed', $host));
        }
        return $host;
    }
    /**
    ...
    private static function filterRegisteredName(string $host) : string
    {
        ...
        if (1 === \preg_match(self::REGEXP_REGISTERED_NAME, $formatted_host)) {
            ...
            if (0 !== $arr['errors']) {
                throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('The host `%s` is invalid : %s', $host, self::getIDNAErrors($arr['errors'])));
            }
            // @codeCoverageIgnoreStart
            if (\false === $unicode) {
        ...
        if (1 !== \preg_match(self::REGEXP_IDN_PATTERN, $formatted_host)) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('Host `%s` is invalid : the host is not a valid registered name', $host));
        }
        // @codeCoverageIgnoreStart
        if (!$idn_support) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\IdnSupportMissing(\sprintf('the host `%s` could not be processed for IDN. Verify that ext/intl is installed for IDN support and that ICU is at least version 4.6.', $host));
        ...
        if ([] === $arr) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('Host `%s` is not a valid IDN host', $host));
        }
        if (0 !== $arr['errors']) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('Host `%s` is not a valid IDN host : %s', $host, self::getIDNAErrors($arr['errors'])));
        }
        // @codeCoverageIgnoreStart
        if (\false === $retval) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\IdnSupportMissing(\sprintf('The Intl extension is misconfigured for %s, please correct this issue before proceeding.', \PHP_OS));
