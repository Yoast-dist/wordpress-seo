...
use YoastSEO_Vendor\League\Uri\Contracts\UriInterface;
use YoastSEO_Vendor\League\Uri\Exceptions\FileinfoSupportMissing;
use YoastSEO_Vendor\League\Uri\Exceptions\IdnSupportMissing;
use YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError;
use YoastSEO_Vendor\Psr\Http\Message\UriInterface as Psr7UriInterface;
...
final class Uri implements \YoastSEO_Vendor\League\Uri\Contracts\UriInterface
{
    /**
     ...
     */
    private const HOST_ADDRESS_BLOCK = "þ€";
    /**
    ...
    private function formatScheme(?string $scheme) : ?string
    {
        ...
        throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('The scheme `%s` is invalid.', $scheme));
    }
    /**
    ...
    private function formatUserInfo(?string $user, ?string $password) : ?string
    {
        ...
        $user = \preg_replace_callback($user_pattern, [\YoastSEO_Vendor\League\Uri\Uri::class, 'urlEncodeMatch'], $user);
        if (null === $password) {
            return $user;
        }
        static $password_pattern = '/(?:[^%:' . self::REGEXP_CHARS_UNRESERVED . self::REGEXP_CHARS_SUBDELIM . ']++|%(?![A-Fa-f0-9]{2}))/';
        return $user . ':' . \preg_replace_callback($password_pattern, [\YoastSEO_Vendor\League\Uri\Uri::class, 'urlEncodeMatch'], $password);
    }
    /**
    ...
    private function formatRegisteredName(string $host) : string
    {
        ...
        if (1 === \preg_match(self::REGEXP_HOST_REGNAME, $formatted_host)) {
            ...
            // @codeCoverageIgnoreStart
            if (!$idn_support) {
                throw new \YoastSEO_Vendor\League\Uri\Exceptions\IdnSupportMissing(\sprintf('the host `%s` could not be processed for IDN. Verify that ext/intl is installed for IDN support and that ICU is at least version 4.6.', $host));
            }
            // @codeCoverageIgnoreEnd
            $unicode = \idn_to_utf8($host, \IDNA_CHECK_BIDI | \IDNA_CHECK_CONTEXTJ | \IDNA_NONTRANSITIONAL_TO_UNICODE, \INTL_IDNA_VARIANT_UTS46, $arr);
            if (0 !== $arr['errors']) {
                throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('The host `%s` is invalid : %s', $host, $this->getIDNAErrors($arr['errors'])));
            }
            // @codeCoverageIgnoreStart
            if (\false === $unicode) {
                throw new \YoastSEO_Vendor\League\Uri\Exceptions\IdnSupportMissing(\sprintf('The Intl extension is misconfigured for %s, please correct this issue before proceeding.', \PHP_OS));
            }
            // @codeCoverageIgnoreEnd
            return $formatted_host;
        }
        if (1 === \preg_match(self::REGEXP_HOST_GEN_DELIMS, $formatted_host)) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('The host `%s` is invalid : a registered name can not contain URI delimiters or spaces', $host));
        }
        // @codeCoverageIgnoreStart
        if (!$idn_support) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\IdnSupportMissing(\sprintf('the host `%s` could not be processed for IDN. Verify that ext/intl is installed for IDN support and that ICU is at least version 4.6.', $host));
        ...
        if ([] === $arr) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('Host `%s` is invalid', $host));
        }
        if (0 !== $arr['errors']) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('The host `%s` is invalid : %s', $host, $this->getIDNAErrors($arr['errors'])));
        }
    ...
    private function formatIp(string $host) : string
    {
        ...
        if (1 === \preg_match(self::REGEXP_HOST_IPFUTURE, $ip, $matches) && !\in_array($matches['version'], ['4', '6'], \true)) {
            return $host;
        }
        $pos = \strpos($ip, '%');
        if (\false === $pos) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('The host `%s` is invalid : the IP host is malformed', $host));
        }
    ...
    private function formatPort($port = null) : ?int
    {
        ...
        if (!\is_int($port) && !(\is_string($port) && 1 === \preg_match('/^\\d*$/', $port))) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('The port `%s` is invalid', $port));
        }
        $port = (int) $port;
        if (0 > $port) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('The port `%s` is invalid', $port));
     ...
     */
    public static function createFromBaseUri($uri, $base_uri = null) : \YoastSEO_Vendor\League\Uri\Contracts\UriInterface
    {
        if (!$uri instanceof \YoastSEO_Vendor\League\Uri\Contracts\UriInterface) {
            $uri = self::createFromString($uri);
        }
        if (null === $base_uri) {
            if (null === $uri->getScheme()) {
                throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('the URI `%s` must be absolute', (string) $uri));
            }
            if (null === $uri->getAuthority()) {
                return $uri;
            }
            /** @var UriInterface $uri */
            $uri = \YoastSEO_Vendor\League\Uri\UriResolver::resolve($uri, $uri->withFragment(null)->withQuery(null)->withPath(''));
            return $uri;
        }
        if (!$base_uri instanceof \YoastSEO_Vendor\League\Uri\Contracts\UriInterface) {
            $base_uri = self::createFromString($base_uri);
        }
        if (null === $base_uri->getScheme()) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('the base URI `%s` must be absolute', (string) $base_uri));
        }
        /** @var UriInterface $uri */
        $uri = \YoastSEO_Vendor\League\Uri\UriResolver::resolve($uri, $base_uri);
        return $uri;
    }
    /**
    ...
    {
        $components = \YoastSEO_Vendor\League\Uri\UriString::parse($uri);
        return new self($components['scheme'], $components['user'], $components['pass'], $components['host'], $components['port'], $components['path'], $components['query'], $components['fragment']);
    }
    /**
    ...
    public static function createFromDataPath(string $path, $context = null) : self
    {
        ...
        // @codeCoverageIgnoreStart
        if (!$finfo_support) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\FileinfoSupportMissing(\sprintf('Please install ext/fileinfo to use the %s() method.', __METHOD__));
        }
        // @codeCoverageIgnoreEnd
        ...
        if (\false === $raw) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('The file `%s` does not exist or is not readable', $path));
        }
        $mimetype = (string) (new \finfo(\FILEINFO_MIME))->file(...$mime_args);
        return \YoastSEO_Vendor\League\Uri\Uri::createFromComponents(['scheme' => 'data', 'path' => \str_replace(' ', '', $mimetype . ';base64,' . \base64_encode($raw))]);
    }
    /**
    ...
    public static function createFromUnixPath(string $uri = '') : self
    {
        ...
        if ('/' !== ($uri[0] ?? '')) {
            return \YoastSEO_Vendor\League\Uri\Uri::createFromComponents(['path' => $uri]);
        }
        return \YoastSEO_Vendor\League\Uri\Uri::createFromComponents(['path' => $uri, 'scheme' => 'file', 'host' => '']);
    }
    /**
    ...
    public static function createFromWindowsPath(string $uri = '') : self
    {
        ...
        if ('' !== $root) {
            return \YoastSEO_Vendor\League\Uri\Uri::createFromComponents(['path' => '/' . $root . $uri, 'scheme' => 'file', 'host' => '']);
        }
        //UNC Windows Path
        if ('//' !== \substr($uri, 0, 2)) {
            return \YoastSEO_Vendor\League\Uri\Uri::createFromComponents(['path' => $uri]);
        ...
        return \YoastSEO_Vendor\League\Uri\Uri::createFromComponents(['host' => $parts[0], 'path' => '/' . $parts[1], 'scheme' => 'file']);
    }
    /**
    ...
    public static function createFromUri($uri) : self
    {
        ...
        if (!$uri instanceof \YoastSEO_Vendor\Psr\Http\Message\UriInterface) {
            throw new \TypeError(\sprintf('The object must implement the `%s` or the `%s`', \YoastSEO_Vendor\Psr\Http\Message\UriInterface::class, \YoastSEO_Vendor\League\Uri\Contracts\UriInterface::class));
        }
        ...
        return new self($scheme, $user, $pass, $host, $uri->getPort(), $uri->getPath(), $query, $fragment);
    }
    /**
    ...
    public static function createFromServer(array $server) : self
    {
        ...
        return \YoastSEO_Vendor\League\Uri\Uri::createFromComponents(['scheme' => self::fetchScheme($server), 'user' => $user, 'pass' => $pass, 'host' => $host, 'port' => $port, 'path' => $path, 'query' => $query]);
    }
    /**
    ...
    private static function fetchUserInfo(array $server) : array
    {
        ...
        if (0 === \strpos(\strtolower($server['HTTP_AUTHORIZATION']), 'basic')) {
            ...
            if (\false === $userinfo) {
                throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError('The user info could not be detected');
            }
    ...
    private static function fetchHostname(array $server) : array
    {
        ...
        if (!isset($server['SERVER_ADDR'])) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError('The host could not be detected');
        }
    ...
    private function formatPath(string $path) : string
    {
        ...
        $path = (string) \preg_replace_callback($pattern, [\YoastSEO_Vendor\League\Uri\Uri::class, 'urlEncodeMatch'], $path);
    ...
    private function formatDataPath(string $path) : string
    {
        ...
        if (\strlen($path) !== \strspn($path, self::ASCII) || \false === \strpos($path, ',')) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('The path `%s` is invalid according to RFC2937', $path));
        }
    ...
    private function assertValidPath(string $mimetype, string $parameters, string $data) : void
    {
        if (1 !== \preg_match(self::REGEXP_MIMETYPE, $mimetype)) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('The path mimetype `%s` is invalid', $mimetype));
        }
        ...
        if ([] !== $res) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('The path paremeters `%s` is invalid', $parameters));
        }
        if (!$is_binary) {
            return;
        }
        ...
        if (\false === $res || $data !== \base64_encode($res)) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('The path data `%s` is invalid', $data));
        }
    }
    /**
    ...
    private function formatQueryAndFragment(?string $component) : ?string
    {
        ...
        static $pattern = '/(?:[^' . self::REGEXP_CHARS_UNRESERVED . self::REGEXP_CHARS_SUBDELIM . '%:@\\/\\?]++|%(?![A-Fa-f0-9]{2}))/';
        return \preg_replace_callback($pattern, [\YoastSEO_Vendor\League\Uri\Uri::class, 'urlEncodeMatch'], $component);
    }
    /**
    ...
    private function assertValidState() : void
    {
        if (null !== $this->authority && ('' !== $this->path && '/' !== $this->path[0])) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError('If an authority is present the path must be empty or start with a `/`.');
        }
        if (null === $this->authority && 0 === \strpos($this->path, '//')) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('If there is no authority the path `%s` can not start with a `//`.', $this->path));
        }
        $pos = \strpos($this->path, ':');
        if (null === $this->authority && null === $this->scheme && \false !== $pos && \false === \strpos(\substr($this->path, 0, $pos), '/')) {
            throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError('In absence of a scheme and an authority the first path segment cannot contain a colon (":") character.');
        }
        ...
        throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('The uri `%s` is invalid for the `%s` scheme.', (string) $this, $this->scheme));
    }
    /**
     ...
     * @param ?string $fragment
     */
    private function getUriString(?string $scheme, ?string $authority, string $path, ?string $query, ?string $fragment) : string
    {
        if (null !== $scheme) {
            $scheme = $scheme . ':';
        }
    ...
    {
        return ['scheme' => $this->scheme, 'user_info' => isset($this->user_info) ? \preg_replace(',:(.*).?$,', ':***', $this->user_info) : null, 'host' => $this->host, 'port' => $this->port, 'path' => $this->path, 'query' => $this->query, 'fragment' => $this->fragment];
    }
    /**
     * {@inheritDoc}
     */
     ...
     * {@inheritDoc}
     */
    public function withScheme($scheme) : \YoastSEO_Vendor\League\Uri\Contracts\UriInterface
    {
    ...
    private function filterString($str) : ?string
    {
        ...
        throw new \YoastSEO_Vendor\League\Uri\Exceptions\SyntaxError(\sprintf('The component `%s` contains invalid characters.', $str));
    }
    /**
     * {@inheritDoc}
     */
    public function withUserInfo($user, $password = null) : \YoastSEO_Vendor\League\Uri\Contracts\UriInterface
    {
        $user_info = null;
     ...
     * {@inheritDoc}
     */
    public function withHost($host) : \YoastSEO_Vendor\League\Uri\Contracts\UriInterface
    {
     ...
     * {@inheritDoc}
     */
    public function withPort($port) : \YoastSEO_Vendor\League\Uri\Contracts\UriInterface
    {
     ...
     * {@inheritDoc}
     */
    public function withPath($path) : \YoastSEO_Vendor\League\Uri\Contracts\UriInterface
    {
     ...
     * {@inheritDoc}
     */
    public function withQuery($query) : \YoastSEO_Vendor\League\Uri\Contracts\UriInterface
    {
     ...
     * {@inheritDoc}
     */
    public function withFragment($fragment) : \YoastSEO_Vendor\League\Uri\Contracts\UriInterface
    {
