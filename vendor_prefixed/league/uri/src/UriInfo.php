...
final class UriInfo
{
    ...
    {
        return $uri instanceof \YoastSEO_Vendor\Psr\Http\Message\UriInterface ? '' : null;
    }
    /**
    ...
    {
        if ($uri instanceof \YoastSEO_Vendor\Psr\Http\Message\UriInterface || $uri instanceof \YoastSEO_Vendor\League\Uri\Contracts\UriInterface) {
            return $uri;
        }
    ...
    private static function normalize($uri)
    {
        ...
        if ('/' === ($path[0] ?? '') || '' !== $uri->getScheme() . $uri->getAuthority()) {
            $path = \YoastSEO_Vendor\League\Uri\UriResolver::resolve($uri, $uri->withPath('')->withQuery($null))->getPath();
        }
        $query = $uri->getQuery();
        ...
        return $uri->withHost(\YoastSEO_Vendor\League\Uri\Uri::createFromComponents(['host' => $uri->getHost()])->getHost())->withPath($path)->withQuery([] === $pairs ? $null : $query)->withFragment($null === $fragmentOrig ? $fragmentOrig : $fragment);
    }
    /**
    ...
    public static function isAbsolutePath($uri) : bool
    {
        $uri = self::filterUri($uri);
        $null = self::emptyComponentValue($uri);
        return $null === $uri->getScheme() && $null === $uri->getAuthority() && '/' === ($uri->getPath()[0] ?? '');
    }
    /**
    ...
    public static function isSameDocument($uri, $base_uri) : bool
    {
        ...
        return (string) $uri->withFragment($uri instanceof \YoastSEO_Vendor\Psr\Http\Message\UriInterface ? '' : null) === (string) $base_uri->withFragment($base_uri instanceof \YoastSEO_Vendor\Psr\Http\Message\UriInterface ? '' : null);
    }
    /**
    ...
    public static function getOrigin($uri) : ?string
    {
        ...
        if ('blob' === $scheme) {
            $uri = \YoastSEO_Vendor\League\Uri\Uri::createFromString($uri->getPath());
